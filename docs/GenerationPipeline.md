...
14. `PresetWriter.WriteGeneratedPreset()` reads the base preset and creates shader adjustments from `ShaderVariableMapper.CreateAdjustments()`.
    ...
    - `Dalashade_MaterialDebug.fx` is an optional false-color utility shader. `Dalashade_SceneGI.fx` is an optional screen-space GI-style shader, not path tracing, RTGI, or PTGI. `Dalashade_SurfaceReflection.fx` is an optional reflection-impression shader, not SSR or ray tracing. Dalashade can inject sections/variables for them, but does not add them to `Techniques=` or require them for normal output.
