...
14. `PresetWriter.WriteGeneratedPreset()` reads the base preset and creates shader adjustments from `ShaderVariableMapper.CreateAdjustments()`.
    ...
    - Material-aware first-party shader behavior is section-scoped: SmartSharpen uses final masks for suppression, AtmosphereBloom uses glow eligibility masks, WeatherAtmosphere uses air/haze masks, AdaptiveGrade uses only subtle protection/preservation masks, SceneGI uses masks for optional contact AO, ambient bounce, and night light pooling, ContactTone uses masks for local grounded edge tone/readability, and SurfaceReflection uses masks for water sheen, wet glints, ice sheen, and neon/aether reflection impressions.
    - Optional Dalapad shader additions are globally gated and surface-data gated. First-party production shaders consume addon data through `Dalashade_FrameData.fxh`; when Dalapad is disabled, unavailable, or stale, FrameData exposes zero Dalapad influence and the shader behaves as if no addon data exists.
