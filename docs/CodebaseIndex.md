...
| Material intent diagnostics | `Dalashade/MaterialIntent.cs`, `Dalashade/MaterialIntentBuilder.cs`, `Dalashade/MaterialIntentContribution.cs` | `MaterialIntent`, `MaterialIntentBuilder`, `MaterialIntentContribution` | Optional inferred material likelihoods and contribution diagnostics | Yes, carefully | Zero-impact by default. Shader variables are written only when MaterialIntent shader mapping is explicitly enabled. |
...
| Material debug shader utility | `shaders/Dalashade_MaterialDebug.fx`, `shaders/Dalashade_MaterialMasks.fxh` | ReShade technique `Dalashade_MaterialDebug` | Optional false-color screen-space material heuristic visualizer for MaterialIntent debugging | Yes, for shader authoring tasks. | Manual ReShade shader-file install/use; disabled by default and never auto-activated. |
