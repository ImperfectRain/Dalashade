...
| File path | Purpose | Runtime role | Inputs | Outputs | Main dependencies | Used by | Status |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `DalapadAddon/README.md` | Safe addon direction and removal boundary. | Documentation only. | Dalapad bridge goals. | Addon implementation guidance, IPC status route, diagnostic control pipe, metadata-only resource catalog, reserved live channel. | `docs/Dalapad.md`. | Future addon work. | Experimental scaffold, separate addon build only. |
| `DalapadAddon/CONTRACT.md` | Human-readable Dalapad bridge contract. | Documentation only. | Candidate G-buffer/depth goals. | Resource names, availability flags, IPC status fields, diagnostic control commands, metadata-only catalog schema, synthetic debug visualization schema, reserved realtime channel, safety rules. | `Dalashade/DalapadDiagnostics.cs` contract names. | Future addon work, diagnostics. | Experimental scaffold, separate addon build only. |
...
| `DalapadAddon/src/dalapad_reshade_addon_skeleton.cpp` | First-test native/ReShade addon source. | Not compiled by Dalashade. | Future separate addon project. | DLL load/unload status-file IPC, diagnostic control-pipe responses, optional ReShade registration when `reshade.hpp` is available, metadata-only unavailable resource catalog rows, synthetic debug texture upload into `Dalapad_Debug.fx`, and disabled realtime channel state. | ReShade/native addon APIs when explicitly prototyped. | Future addon work. | Experimental scaffold, not wired into the plugin build. |
...
| Change Dalapad addon scaffold | `DalapadAddon/README.md`, `DalapadAddon/CONTRACT.md`, `DalapadAddon/dalapad-addon-contract.json`, `DalapadAddon/src/dalapad_reshade_addon_skeleton.cpp`, `shaders/Dalapad_Debug.fx`, `docs/Dalapad.md` |
