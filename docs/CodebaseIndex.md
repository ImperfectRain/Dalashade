...
| `Dalashade/SceneAuthoring/SceneAuthoringService.cs` | Scene authoring override service. | Loads/saves current-territory tag overrides and tag preset metadata, applies overrides after automatic classification, emits suppression maps/conflict warnings, and handles fixed-path import/export. | Plugin config folder, `GameContext`, detected `SceneTags`, UI edits. | Effective `SceneTags`, authoring state, override JSON, tag preset JSON. | System.Text.Json. | Plugin, SceneAuthoringWindow, reports. | Experimental. |
| `Dalashade/SceneAuthoring/SceneTagOverride.cs` | Scene authoring models. | Stores override sets, tag preset metadata, per-territory added/removed tag groups, primary biome overrides, warnings, and applied state. | JSON storage, UI edits. | Serializable override, preset, and diagnostics state. | SceneAuthoringService. | Plugin/UI/reports. | Experimental. |
...
| `Dalashade/Windows/SceneAuthoringWindow.cs` | Scene authoring UI. | Separate window for viewing detected/effective tags, enabling overrides, adding/removing grouped current-scene tags, and resetting current scene overrides. | Plugin scene authoring state. | ImGui UI and override edits. | SceneAuthoringService. | User/developer scene authoring. | Experimental. |
