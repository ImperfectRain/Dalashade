...
FrameData diagnostics report the internal resolver contract state. Current expected status is `FrameDataMode: Inline`, `FrameDataPrepass: NotImplemented`, `ProductionFrameDataConsumers: WeatherAtmosphere`, and `ProductionShadersMigratedToFrameData: Dalashade_WeatherAtmosphere.fx`. The report also shows whether `Dalashade_FrameData.fxh` and `Dalashade_FrameDataDebug.fx` are available to the source scan, whether the generated preset contains a FrameDataDebug section, whether the technique appears active, and the FrameDataDebug debug variables. WeatherAtmosphere uses inline FrameData; other production shaders remain unmigrated until explicit migration passes.
