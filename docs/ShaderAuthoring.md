...
For the short first-party shader contract, start with [ShaderContractQuickReference.md](ShaderContractQuickReference.md). It defines which shared include owns FrameData, Dalapad, MaterialMasks, and NormalField behavior, and the required neutral-fallback rules.
...
- `Configuration.FirstPartyPerformanceTier` lets users choose Quality, Balanced, or Performance for first-party production shaders. Quality preserves current generated values and full shader helper paths; lower tiers trim optional helper work only.
...
- Optional local contact tone pass for grounded edge darkening, material readability contrast, and scene-safe local separation. It is not GI, SSAO, reflection, or bloom.
...
| First-party performance tier uniforms | `Dalashade_FirstPartyPerformanceTier`, `Dalashade_GISampleCountScale`, `Dalashade_GISampleDistanceScale`, `Dalashade_ReflectionSampleQuality`, `Dalashade_BloomSampleQuality` | Yes, for known first-party production shader sections when custom shader support is enabled and matching keys exist or generated-preset section injection adds them. |
...
| Shader-owned controls | `Dalashade_EnableDepthAssist`, `Dalashade_DepthAssistStrength`, `Dalashade_DepthAssistConfidenceFloor`, `Dalashade_DepthConfidenceFloor`, MaterialDebug debug mode/opacity/strength controls | No active writes. These may be known or injected with default values, but users control them in ReShade. SceneGI, ContactTone, and SurfaceReflection controls/debug modes are the exception: they can be written by their dedicated variable-write toggles. |

## First-party performance tiers

The first-party shader quality dropdown controls optional helper budgets for Dalashade production shaders. It does not enable techniques, install shader files, or bypass shader safety gates.

| Tier | Behavior |
| --- | --- |
| Quality | Default. Preserves current generated values and full helper paths. SceneGI keeps its full diffuse gather, SurfaceReflection keeps all projection/pseudo-SSR helpers, AtmosphereBloom keeps the full bloom gather, and NormalField influence is unchanged. |
| Balanced | Trims expensive optional helpers while keeping the look close to Quality. NormalField detail/relief and material influence are modestly reduced, SceneGI uses fewer diffuse-gather taps and shorter generated sample distance/radii, SurfaceReflection skips far/planar extras, AtmosphereBloom skips its far ring, and ContactTone uses a modestly smaller generated radius. |
| Performance | Prefers cheaper paths and lets authorized Dalapad surface data matter more by reducing inferred NormalField work. SceneGI uses its smallest diffuse-gather set, reflection projection/pseudo-SSR helpers favor near/local paths, NormalField influence is lower, and generated GI/reflection/contact radii are reduced. It does not increase visual intensity to compensate. |

Generated performance values include `Dalashade_FirstPartyPerformanceTier`, `Dalashade_GISampleCountScale`, `Dalashade_GISampleDistanceScale`, `Dalashade_ReflectionSampleQuality`, and `Dalashade_BloomSampleQuality`. Existing generated NormalField, SceneGI, ContactTone, and SurfaceReflection uniforms may also be tier-scaled. Missing sections or uniforms are skipped safely.
...
The generated SceneGI section includes conservative GI controls and `Dalashade_Intent*` aliases for the SceneIntent values the shader consumes. `EnableDalashadeSceneGIShaderVariables` controls whether Dalashade actively rewrites those GI controls during generation; technique activation remains manual. First-party performance tiers can also write `Dalashade_GISampleCountScale` and `Dalashade_GISampleDistanceScale` so lower tiers reduce optional diffuse-gather taps and generated GI radius/distance. When MaterialIntent shader mapping is enabled, SceneGI may also receive `Dalashade_MaterialFoliage`, `Dalashade_MaterialWaterPlane`, `Dalashade_MaterialSpecularGlint`, `Dalashade_MaterialSandDust`, `Dalashade_MaterialSnowIce`, `Dalashade_MaterialStoneRuins`, `Dalashade_MaterialMetalIndustrial`, `Dalashade_MaterialCrystalAether`, `Dalashade_MaterialNeonGlass`, `Dalashade_MaterialFireLavaHeat`, `Dalashade_MaterialSkyCloudFog`, `Dalashade_MaterialSkinProtection`, and `Dalashade_MaterialVoidDarkness`.

The generated ContactTone section includes local contact/readability controls and the SceneIntent values the shader consumes. `EnableDalashadeContactToneShaderVariables` controls whether Dalashade actively rewrites those controls during generation; generated section default keeps `Dalashade_ContactToneEnabled=0` so section injection alone is visually inert. When MaterialIntent shader mapping is enabled, ContactTone may also receive `Dalashade_MaterialFoliage`, `Dalashade_MaterialWaterSpecular`, `Dalashade_MaterialWaterPlane`, `Dalashade_MaterialSpecularGlint`, `Dalashade_WaterContext`, `Dalashade_WetSurfaceContext`, `Dalashade_MaterialSandDust`, `Dalashade_MaterialSnowIce`, `Dalashade_MaterialStoneRuins`, `Dalashade_MaterialMetalIndustrial`, `Dalashade_MaterialSkyCloudFog`, `Dalashade_MaterialSkinProtection`, and `Dalashade_MaterialVoidDarkness`.

The generated SurfaceReflection section includes reflection-impression controls and the SceneIntent values the shader consumes. `EnableDalashadeSurfaceReflectionShaderVariables` controls whether Dalashade actively rewrites those reflection controls during generation; technique activation remains manual. First-party performance tiers can also write `Dalashade_ReflectionSampleQuality` and tier-scaled offset/softness values so lower tiers skip optional far projection and pseudo-SSR helper samples. When MaterialIntent shader mapping is enabled, SurfaceReflection may also receive `Dalashade_MaterialWaterPlane`, `Dalashade_MaterialSpecularGlint`, `Dalashade_WaterContext`, `Dalashade_CoastalContext`, `Dalashade_OpenOceanContext`, `Dalashade_ShallowWaterContext`, `Dalashade_WetSurfaceContext`, `Dalashade_MaterialSandDust`, `Dalashade_MaterialSnowIce`, `Dalashade_MaterialMetalIndustrial`, `Dalashade_MaterialCrystalAether`, `Dalashade_MaterialNeonGlass`, `Dalashade_MaterialFireLavaHeat`, `Dalashade_MaterialSkyCloudFog`, and `Dalashade_MaterialSkinProtection`.

Dalashade also does not currently install or copy custom shader files into a ReShade shader directory. Generated-preset section injection can add known sections and variables, while technique activation remains manual unless the separate `SyncDalashadeTechniqueActivation` option is enabled. For manual testing, place the needed files from `shaders/` somewhere ReShade scans for shaders, then enable wanted techniques in ReShade.
...
| Technique activation synced | Expected to be `no` unless `SyncDalashadeTechniqueActivation` is enabled. Auto-injection alone adds sections and variables only; activation sync may add or remove known production first-party techniques in `Techniques=`. |
| Generated preset only | Injection happened in the generated output path; the base preset was not modified. |
| Base preset contains a Dalashade custom shader section | The selected base preset has a section such as `[Dalashade_WeatherAtmosphere.fx]`, `[Dalashade_AdaptiveGrade.fx]`, `[Dalashade_SmartSharpen.fx]`, `[Dalashade_AtmosphereBloom.fx]`, `[Dalashade_MaterialDebug.fx]`, `[Dalashade_SceneGI.fx]`, `[Dalashade_ContactTone.fx]`, or `[Dalashade_SurfaceReflection.fx]`, so Dalashade can inspect it for supported `Dalashade_*` keys. |
...
Technique activation sync is disabled by default. Dalashade can inject known sections and variables into the generated preset without enabling techniques, or it can opt-in manage Dalashade production entries in `Techniques=` when `SyncDalashadeTechniqueActivation` is enabled. The sync allow-list is intentionally limited to production first-party shaders such as AdaptiveGrade, SceneGI, ContactTone, SurfaceReflection, WeatherAtmosphere, AtmosphereBloom, and SmartSharpen. Manual diagnostic shaders, including `Dalashade_MaterialDebug.fx`, `Dalashade_NormalDebug.fx`, `Dalashade_FrameDataDebug.fx`, and `Dalapad_Debug.fx`, are not auto-enabled by technique sync. Base preset technique inactive/unknown means only that the base preset did not confirm activation. Users must still install the relevant `.fx` files through ReShade shader search paths.
...
| `Dalashade_GIDebugMode` | Integer enum: `0` normal, `1` AO, `2` bounce, `3` night light pooling, `4` material influence, `5` sky rejection, `6` skin protection, `7` final GI influence, `8` depth-normal confidence, `9` emissive source, `10` bounce receiver, `11` adaptive limits/safety, `12` layered AO breakdown, `13` clamp pressure, `14` SSGI diffuse gather, `15` material bounce lanes, `16` sky-safe receivers, `17` emissive pooling lanes, `18` Dalapad used contribution, `19` Dalapad FrameData evidence, `20` Dalapad bridge gate, `21` Dalapad raw normal sample. Dalashade writes it as `0` through `21`, not as a normalized float. |
...
| `Dalashade_FoliageDensity` | Allows canopy gap bloom only when local bright openings are surrounded by foliage-like texture. |
...
| `Dalashade_MaterialFoliage` | Supports canopy gap bloom when MaterialIntent mapping is enabled; it does not bloom broad sky by itself. |
...
| `CanopyGapBloomStrength` | Default `0.34`. Extra source strength for bright sky/light openings surrounded by foliage-like samples, with broad-sky rejection. |
...
| `Dalashade_MaterialDebugMode` | Material-aware debug override: `0` off, `1` overview, `2` water/specular, `3` crystal/aether, `4` neon/glass, `5` fire/heat, `6` sky/fog, `7` final bloom eligibility, `8` water plane, `9` specular glint, `10` canopy gap bloom. |
| `Dalashade_MaterialDebugStrength` | Scales material debug-mask visibility. Debug masks show inferred influence, not true engine material IDs. |

The shader uses a single-pass fixed sample ring rather than a large multi-pass blur. It is meant for cheap scene-aware glow, not large full-screen bloom. Bright coastal scenes should keep clean specular sparkle without nuclear sand/sky bloom; jungle scenes should get subtle bloom from small bright canopy openings only when the center pixel is bright, nearby samples look darker/foliage-like, and broad smooth sky is rejected; rain should pick up small wet highlights; neon and magic should tint local luminous sources; combat and readability pressure should cut strength quickly. At night, bloom becomes more selective: ambient darkness raises restraint, artificial light lowers eligibility only around bright local pools, and moonlight adds only subtle atmospheric diffusion. MaterialIntent only makes bloom eligibility more selective: foliage supports canopy gaps, water/specular softens glints, crystal/aether and neon/glass require color/luma cues, fire/heat favors warm sources, and sky/fog gets extra wash restraint.
...
NormalDebug visualizes inferred screen-space depth normals, detail normals, texture-relief normals, combined normals, ground/wall facing masks, normal confidence, shading receivers, reflection receivers, AO receivers, and safety suppression. It is not true game material normals, FFXIV G-buffer access, or real normal maps. Texture relief treats local bright high-pass detail as ridges and local dark high-pass detail as grooves only after material/safety gates and broad-gradient rejection. Mode `0` and disabled `Dalashade_NormalDebugEnabled` are pass-through for normal gameplay.
