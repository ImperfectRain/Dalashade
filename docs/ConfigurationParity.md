...
| SceneGI contact shadows: enable, strength, radius, softness | off, 0.30/0.62/0.55 | yes | yes | yes when SceneGI writes enabled | registry-backed | bundle via generated-variable summaries | yes | indirect | Local AO/contact grounding only. Separate from ScreenShadows. |
| ScreenShadows controls: enable writes, strength, reach, softness, source sensitivity, Dalapad assist | off, conservative defaults | yes | yes | yes when enabled | registry-backed | bundle via generated-variable summaries | yes | optional assist | Separate production technique. Default-off and approximate screen-space source shadowing. |
| ScreenShadows debug controls: mode, output, opacity, boost | off/full replacement defaults | no | yes | yes when ScreenShadows writes enabled | registry-backed | bundle via generated-variable summaries | debug only | optional evidence view | Debug modes show shader evidence, not true engine light/shadow maps. |
