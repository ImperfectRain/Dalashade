...
public sealed class DebugBundleExporter
{
    ...
    private static object BuildDalapadDiagnosticsDump(DalapadDiagnostics diagnostics)
    {
        return new
        {
            ...
            diagnostics.ControlPipeStatus,
            diagnostics.ResourceShapeProbe,
            ...
            Contract = new
            {
                ...
                InvokesRenderTargetManagerInstance = diagnostics.ResourceShapeProbe.InstanceInvoked,
                SyntheticDebugTextureUploaded = diagnostics.ControlPipeStatus.DebugVisualization.SyntheticTextureUploaded,
                DebugVisualizationReadsRenderTargets = diagnostics.ControlPipeStatus.DebugVisualization.ReadsRenderTargets,
                DebugVisualizationCopiesRenderTargets = diagnostics.ControlPipeStatus.DebugVisualization.CopiesRenderTargets,
                DebugVisualizationRegistersGameResources = diagnostics.ControlPipeStatus.DebugVisualization.RegistersGameResources,
                ProvidesReShadeShaderSampling = false,
                OpensNamedPipe = diagnostics.ControlPipeStatus.PipeListening,
...
dalapad-diagnostics.json records the diagnostic-only Dalapad surface-data probe. It reports runtime metadata, optional status-file IPC availability, optional short-timeout control-pipe capability negotiation, an explicitly enabled developer-only resource shape probe, synthetic debug visualization status, and debug-only addon-owned render-layer copy status when available. The shape probe may invoke RenderTargetManager.Instance, and the addon may upload generated pixels or copied diagnostic textures into Dalapad_Debug.fx, but this build does not send texture handles over IPC, move shader values, or change shader/preset behavior.
